...
class JLogicalOrOp extends JBooleanBinaryExpression {
    ...
    public void codegen(CLEmitter output, String targetLabel, boolean onTrue) {
        // Added for Problem 4 Exercise 5.12
    	if (onTrue) {
...
/**
 * The AST node for a logical Not Equal (!=) expression. Implements short-circuiting
 * branching.
 */
...
class JNotEqualOp extends JBooleanBinaryExpression {
    ...
    /**
     * Construct an AST node for a logical Not Equal expression given its line number,
     * and lhs and rhs operands.
     * 
     * @param line
     *            line in which the logical AND expression occurs in the source
    ...
    /**
     * Analyzing a logical Not Equal expression involves analyzing its operands and
     * insuring they are boolean; the result type is of course boolean.
     * 
     * @param context
     *            context in which names are resolved.
