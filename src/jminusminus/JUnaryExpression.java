...
class JUnaryPositiveOp extends JUnaryExpression {
    ...
    public JUnaryPositiveOp(int line, JExpression arg) {
        super(line, "+", arg);
    }
    ...
    /**
     * Generating code for the negation operation involves generating code for
     * the operand, and then the unary positive instruction.
     * 
     * @param output
     *            the code emitter (basically an abstraction for producing the
     *            .class file).
...
/**
 * JOSH
 * The AST node for a unary Bitwise Compliment (~) expression.
 */

class JBitCompOp extends JUnaryExpression {
    ...
    public JBitCompOp(int line, JExpression arg) {
        super(line, "~", arg);
    }
...
//            output.addNoArgInstruction(ICONST_1);
//            output.addNoArgInstruction(ISUB);
//            ((JLhs) arg).codegenStore(output);//TODO
//        }
    }
